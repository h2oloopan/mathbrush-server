...
relation::relation(const std::string &name_, size_t n_, double t0_, double t1_, double t2_, get_angle_fn t_)
	: name(name_), n(n_), t0(t0_), t1(t1_), t2(t2_), t(t_)
	{ }
...
initialize_relations()
{
	...
	for (unsigned i = 0; i < NUM_RELATIONS - 1; ++i) {
		std::string name;
		size_t n;
		double t0, t1, t2;
		relin >> name;
		CHECK_ISTREAM(relin, "error reading relation data");
			...
			relations[link_name_to_index(name)] = new relation(name, n, t0, t1, t2, angle_fns[name]);
		}
	}
	relations[NUM_RELATIONS - 1] = new C_rel();
}
...
destroy_relations()
{
	std::string path;
	GetTrainingPath(path);
	std::ofstream relout((path + "/reldata").c_str());
	if (!relout.is_open()) {
		THROW_ERROR(E_NOTFOUND, "could not open reldata file");
	}
	for (unsigned i = 0; i < NUM_RELATIONS - 1; ++i) {
		relout << relations[i]->name << ' ' << relations[i]->n << ' '
		       << relations[i]->t0 << ' ' << relations[i]->t1 << ' ' << relations[i]->t2 << std::endl;
	}
